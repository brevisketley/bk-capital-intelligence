...
def _research_critic(o: Opportunity) -> Critique:
    ...
    if o.confidence < 0.60:
        findings.append("evidence confidence below 0.60")
    if not o.source or o.source == "unknown":
        findings.append("source provenance is missing")
    if o.base_apy is not None and o.reward_apy is not None and o.gross_apy > 0:
        reward_share = max(0.0, o.reward_apy) / o.gross_apy
        if reward_share > 0.80:
...
def _liquidity_critic(o: Opportunity) -> Critique:
    findings = []
    if o.liquidity_usd <= 0:
        findings.append("no usable liquidity reported")
    elif o.tvl_usd > 0 and o.liquidity_ratio < 0.05:
        findings.append("liquidity is less than 5% of TVL")
    if o.lockup_days > 30:
        findings.append("exit is materially constrained by lockup")
...
def _risk_critic(o: Opportunity, risk: RiskAssessment) -> Critique:
    findings = []
    if risk.blocked:
        findings.append("risk engine has already blocked the opportunity")
    if risk.score < 0.45:
        findings.append(f"risk-adjusted score {risk.score:.3f} is below minimum quality")
    if o.leverage > 1.5:
        findings.append("leverage exceeds conservative review threshold")
    return Critique("risk", "critical" if risk.blocked else ("high" if findings else "info"), not findings, tuple(findings))
...
def _concentration_critic(opportunities: list[Opportunity], weights: dict[str, float]) -> Critique:
    ...
    if max(by_protocol.values(), default=0.0) > 0.40:
        findings.append("protocol concentration exceeds 40%")
    if max(by_chain.values(), default=0.0) > 0.60:
        findings.append("chain concentration exceeds 60%")
...
def review(opportunities: list[Opportunity], risks: list[RiskAssessment], weights: dict[str, float]) -> AdversarialReview:
    """Run independent critics and fail closed on critical/high findings."""
