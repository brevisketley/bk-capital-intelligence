...
def run(opportunities: list[Opportunity], policy: GuardianPolicy = GuardianPolicy()) -> SwarmResult:
    ...
    findings = tuple(ResearchFinding(o.opportunity_id, o.confidence, (f"source={o.source}", f"apy={o.gross_apy:.6f}", f"tvl_usd={o.tvl_usd:.2f}")) for o in opportunities)
    ...
    if not adversarial.passed:
        guardian = tuple(PolicyDecision(g.opportunity_id, False, tuple(list(g.reasons) + ["adversarial review failed"]), g.risk) for g in guardian)
